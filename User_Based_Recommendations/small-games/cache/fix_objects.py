...
expected_profiles = open('./objects/all.txt', 'r').readlines()
for i in range(len(expected_profiles)):
    expected_profiles[i] = expected_profiles[i][29:-1]
#    print (i[28:-1])

existing_profiles = os.listdir('./objects/')
for i in range(len(existing_profiles)):
    existing_profiles[i] = existing_profiles[i][:-4]
#    print(i[:-4])

existing_profiles.remove('all')
f = open('./all_objects_profiles.txt', 'w')
